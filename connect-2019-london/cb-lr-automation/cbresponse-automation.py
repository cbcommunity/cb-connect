...
from utils.cbresponse.triggers import NotificationListener, SyntheticNotificationGenerator
from utils.orchestrator import LiveResponseOrchestrator

log = logging.getLogger(__name__)


def get_lr_session_response(cb, sensor_id):
...
def main():
    ...
    if args.synthetic:
        ...
        notification_thread = NotificationListener(datastore, poll_interval=args.poll)
    notification_thread.daemon = True
    notification_thread.start()

    cb = get_cb_response_object(args)

    orchestrator = LiveResponseOrchestrator(cb, get_lr_session_response, CollectContextWorker, datastore)
    orchestrator.start()
